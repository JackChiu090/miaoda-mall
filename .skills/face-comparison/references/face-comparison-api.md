...
| Endpoint | `POST https://app-cj6aqssgkd8h-api-5YrZz81oerkY-gateway.appmiaoda.com/rest/2.0/face/v3/match` |
| Auth 模式 | `platform_managed`（密钥由平台注入） |
| Auth Header | `X-Gateway-Authorization: Bearer ${apiKey}` |
| Content-Type | `application/json` |
| third_part_domain | `app-cj6aqssgkd8h-api-5YrZz81oerkY-gateway.appmiaoda.com` |
| 计费 | 启用，按调用次数计费 |
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-cj6aqssgkd8h-api-5YrZz81oerkY-gateway.appmiaoda.com/rest/2.0/face/v3/match",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
