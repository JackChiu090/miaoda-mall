...
| Endpoint | `GET https://app-cj6aqssgkd8h-api-o9wN0pyVE2ea-gateway.appmiaoda.com/v1/videos/omni-video/{id}` |
| Auth 模式 | `platform_managed`（`traefik: true`） |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| Content-Type | `application/json` |
| third_part_domain | `app-cj6aqssgkd8h-api-o9wN0pyVE2ea-gateway.appmiaoda.com` |
...
serve(async (req: Request): Promise<Response> => {
  ...
  let queryUrl: string;
  if (taskId) {
    queryUrl = `https://app-cj6aqssgkd8h-api-o9wN0pyVE2ea-gateway.appmiaoda.com/v1/videos/omni-video/${taskId}`;
  } else {
    queryUrl = `https://app-cj6aqssgkd8h-api-o9wN0pyVE2ea-gateway.appmiaoda.com/v1/videos/omni-video/${externalTaskId}?external_task_id=${externalTaskId}`;
  }
