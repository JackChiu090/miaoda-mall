...
| Endpoint | `GET https://app-cj6aqssgkd8h-api-ra5ErGpGM8wa-gateway.appmiaoda.com/traffic/v1/bound` |
| Auth | `platform_managed`（`X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}`） |
| Content-Type | `application/json`（GET 请求，参数通过 Query String 传递） |
| third_part_domain | `app-cj6aqssgkd8h-api-ra5ErGpGM8wa-gateway.appmiaoda.com` |
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-cj6aqssgkd8h-api-ra5ErGpGM8wa-gateway.appmiaoda.com/traffic/v1/bound?${params.toString()}`,
    {
      method: "GET",
      headers: {
