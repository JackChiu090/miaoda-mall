...
| Endpoint | `POST https://app-cj6aqssgkd8h-api-zYkZz8qovQ1L-gateway.appmiaoda.com/v2/chat/completions` |
| Content-Type | `application/json` |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| 认证模式 | `platform_managed`（密钥由平台注入，读取 `INTEGRATIONS_API_KEY`） |
| Third-party Domain | `app-cj6aqssgkd8h-api-zYkZz8qovQ1L-gateway.appmiaoda.com` |
| 响应模式 | SSE 流式 |
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-cj6aqssgkd8h-api-zYkZz8qovQ1L-gateway.appmiaoda.com/v2/chat/completions",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
