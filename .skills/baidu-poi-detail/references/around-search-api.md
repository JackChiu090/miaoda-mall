...
| Endpoint | `GET https://app-cj6aqssgkd8h-api-DLEO7eMnzMwa-gateway.appmiaoda.com/place/v3/around` |
| Auth | `X-Gateway-Authorization: Bearer <INTEGRATIONS_API_KEY>`（platform_managed） |
| Content-Type | 无请求体（GET 参数通过 URL 查询串传递） |
| Third-party domain | `app-cj6aqssgkd8h-api-DLEO7eMnzMwa-gateway.appmiaoda.com` |
| 计费 | 按次计费 |
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-cj6aqssgkd8h-api-DLEO7eMnzMwa-gateway.appmiaoda.com/place/v3/around?${params.toString()}`,
    {
      method: "GET",
      headers: {
