...
| Endpoint | `https://app-cj6aqssgkd8h-api-eLMlJ2jB4oj9-gateway.appmiaoda.com/enterprise/patent/detail` |
| HTTP Method | POST |
| Content-Type | `application/json;charset=UTF-8` |
| Auth 模式 | platform_managed |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| third_part_domain | `app-cj6aqssgkd8h-api-eLMlJ2jB4oj9-gateway.appmiaoda.com` |
| 计费 | 未启用（免费接口） |
...
serve(async (req: Request): Promise<Response> => {
  ...
  // --- 调用上游 API ---
  const url = new URL(
    "https://app-cj6aqssgkd8h-api-eLMlJ2jB4oj9-gateway.appmiaoda.com/enterprise/patent/detail",
  );
